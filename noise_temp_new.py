...
def I_kp_perp_kperp(kperp,kp_perp,kpar,kp_par,z1,Noise):
    ...
    integrand_P_delv_N_delv_2_arr=np.array([])
    theta_kp=u
    chi_z1=chi(z1)
    ...
    for i in range(n):
        for j in range(n):
            ...
            integrand_P_delv_N_delv_2_arr=np.append(integrand_P_delv_N_delv_2_arr,integrand_P_delv_N_delv_2)
            #integrand_arr[i,j]=sp.integrate.trapz(integrand_P_deldel_N_vv,u)
            integrand_arr[i,j]=trapz(integrand_P_vv_N_deldel,u)
    return integrand_arr, integrand_P_delv_N_delv_2_arr, Noise_Kperp_arr, K_perp_arr
...
pylab.pcolormesh(kp_perp,kperp, Z[0], vmin=(Z[0]).min(), vmax=10000, norm=LogNorm()); cbar=plt.colorbar()
...
plt.title(r'$P_{\delta\delta} N_{vv}, k_\parallel=k_{p_\parallel}=10^{-1}$',fontsize=14)
plt.xscale('log')
plt.yscale('log')
plt.show()
...
def PN_integrals_integral_over_y(ell,z1,delta_z, Noise):
    ...
    Z_max=z1+delta_z
    Z=np.geomspace(Z_min,Z_max,n)
    u,kp,kpar,z2=np.meshgrid(U,Kp,Kpar,Z)
    ...
    for i in ell:
        ...
        integral_P_deldel_P_vv=const*trapz(trapz(trapz(trapz(integrand_P_deldel_P_vv,U,axis=0),Kp,axis=0),Kpar,axis=0),Z,axis=0)
        integral_P_delv_P_delv=const*trapz(trapz(trapz(trapz(integrand_P_delv_P_delv,U,axis=0),Kp,axis=0),Kpar,axis=0),Z,axis=0)
        integral_P_vv_N_deldel=const*trapz(trapz(trapz(trapz(integrand_P_vv_N_deldel,U,axis=0),Kp,axis=0),Kpar,axis=0),Z,axis=0)
        integral_P_deldel_N_vv=const*trapz(trapz(trapz(trapz(integrand_P_deldel_N_vv,U,axis=0),Kp,axis=0),Kpar,axis=0),Z,axis=0)
        integral_P_delv_N_delv_1=const*trapz(trapz(trapz(trapz(integrand_P_delv_N_delv_1,U,axis=0),Kp,axis=0),Kpar,axis=0),Z,axis=0)
        integral_P_delv_N_delv_2=const*trapz(trapz(trapz(trapz(integrand_P_delv_N_delv_2,U,axis=0),Kp,axis=0),Kpar,axis=0),Z,axis=0)
        integral_N_deldel_N_vv=const*trapz(trapz(trapz(trapz(integrand_N_deldel_N_vv,U,axis=0),Kp,axis=0),Kpar,axis=0),Z,axis=0)
        integral_N_delv_N_delv=const*trapz(trapz(trapz(trapz(integrand_N_delv_N_delv,U,axis=0),Kp,axis=0),Kpar,axis=0),Z,axis=0)
